...
class CatPlayer():
    ...
    def validRandomMove(self,mousePosition):
        ...
        while True:
            ...
            origin = min(self.cats[0], self.cats[1], self.cats[2], self.cats[3]) #Calcula la posicion minima de los gatos
            ...
            if target == int(mousePosition):
                if origin == self.cats[0]:
                    opt = self.cats[1]
                if origin == self.cats[1]:
                    opt = self.cats[2]
                if origin == self.cats[2]:
                    opt = self.cats[3]
                if origin == self.cats[3]:
                    opt = self.cats[0]
                origin = opt
